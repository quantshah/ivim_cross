...
def leave_one_cross(model, data_slice, gtab, mask=None):
    """
    ...
    for left_out in range(1, data_slice.shape[-1]):
        ...
        fit = current_model.fit(left_out_data, mask)
...
predicted_ivim = leave_one_cross(IvimModel, data, gtab, mask)

predicted_exp = np.zeros_like(data)
predicted_exp = leave_one_cross(ExponentialModel, data, gtab, mask)
